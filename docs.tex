...
$$ a^k = b \text{ com } G = \langle a \rangle, |G| = n, b \in G$$
...
\begin{align*}
    ...
    k &= k_m \mod p_m^{e_m}
\end{align*}
A implementação calcula $k_i$ para cada fator primo de $n$ com auxílio da função Pohlig-Hellman para grupos de ordem primária, então calcula $k$ utilizando o algorítmo para o teorema Chinês do resto em $O(m)$
...
$$ a^k = b \text{ com } G = \langle a \rangle, |G| = p^e, b \in G$$
...
$$ a^k \equiv b \bmod p \text{ com } G = \langle a \rangle, |G| = n, b \in G$$
