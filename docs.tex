...
Dessa forma, cada teste tem complexidade $O(m)$, no qual $m$ é o número de fatores primos distintos de $p-1$. O menor gerador de um grupo foi provado por Pretorius(2018) ser $g_p < p^{0.68}$. Como o algoritmo realiza os testes enquanto não achar um gerador, a complexidade é $O(m \cdot p ^{0.68})$. Porém o algoritmo é bem mais rápido na prática, uma vez que é relativamente comum se ter geradores pequenos para esses grupos, dado que já se tem a fatoração de $p-1$.
...
ord(b_j) &\mid p_j^{e_j} \text{ pois, } b_j^{p_j^{e_j}} =  a^{p-1} \equiv 1 \bmod p\\
b_j ^ {p_j^{e_j - 1}} = a ^ {(p-1)/p_j} &\neq 1 \implies ord(b_j)\nmid p_j^{e_j-1}\therefore ord(b_j) = p_j^{e_j}
\end{align*}
...
Já quando temos apenas a fatoração parcial, fazemos o mesmo processo para os fatores $p_i^{e_i}$ conhecidos e para o fator desconhecido $N$, porém aqui não saberemos a ordem exata para este elemento, apenas que está entre $q\cdot \frac{p-1}{N}$ e $p-1$, na qual $q$ é o próximo primo maior que o limite imposto na fatoração parcial ($2^{20}$), i.\,e.\, $q = 1048583$. 
...
Observe que a cada fator primo distinto de $p-1$ temos que encontrar um elemento de ordem $p_i^{e_i}$ sequencialmente, porém não foi encontrado um limite superior na literatura para essa busca e por isso chamaremos de $O(k)$. Portanto a complexidade do algoritmo acima é $O(\sum_{i=1}^{m}\cdot k \cdot \log_2{p_i^{e_i}})$.

Note que se considerássemos a escolha do elemento de forma aleatória, teríamos $k$ "limitado" por uma constante. Isso ocorre porque existem $\frac{p-1}{q}$ raízes que satisfazem a equação $x^{(p-1)/q}\equiv 1$, e logo uma probabilidade de $\frac{1}{q}$ de escolher um que satisfaça, porém no algoritmo queremos o contrário, ou seja, números que não satisfaçam a equação anterior, e a probabilidade de escolher um desses é $1 - \frac{1}{q}$. Dessa forma, a probabilidade de falhar nessa busca em $k$ tentativas é $\frac{1}{q^k}$, e podemos tomar $k = 20$ que temos uma probabilidade quase certa de encontrar o elemento que queremos.
